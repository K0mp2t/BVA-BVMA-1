...
class Attack: 
    ...
    def Group_cluster(self):
        ...
        with open(os.path.join(utils.DATASET_PATH, "{}_wl_v_off_SampleDataOfShieldDB.pkl".format(dataset_name.lower())), "rb") as f:
            _, sample_length = pickle.load(f)#_after_padding_2
            f.close()
        keyword_identifier = [0]*len(chosen_kws)
        keyword_length = [0]*len(chosen_kws)
        for k in sample_length.keys():#range(len(keyword_identifier)):
            keyword_identifier[k] = k
            #if k in sample_length.keys():
            keyword_length[k] = sample_length[k]
        _, new_keyword_identifier = zip(*sorted(zip(keyword_length, keyword_identifier)))
