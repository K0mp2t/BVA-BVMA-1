...
import attacks
def multiprocess_worker(dataset_name, begin_time, observed_period, target_period, number_queries_per_period, adv_observed_offset, gamma):
...
def plot_figure(dataset_name):
    ...
    if dataset_name=='Enron':
        ...
        ax.set_ylabel('Injection size', fontsize = textsize)
        ax.set_yscale('log')
        ax.tick_params(labelsize = textsize)
        ax.legend(loc = 'center', fontsize = textsize)

        ax2 = ax.twinx()
        ax2.plot(x_off[:len(x_off)-2]+[x_off[len(x_off)-1]], BVA_accuracy[:len(BVA_injection_size)-2]+[BVA_accuracy[len(BVA_accuracy)-1]], 'lightgreen', marker = '^', markersize=10, markeredgecolor = 'g', markeredgewidth=0.8, label = 'BVA-Rer')
        ...
        xytext=(x_off[0]-.01, BVA_accuracy[0]-.05), color = 'black', fontsize=textsize,
        arrowprops=dict(arrowstyle="->"))
        ax2.annotate('(offset, {:.2f})'.format(BVA_accuracy[len(BVA_accuracy)-1]),
        xy=(x_off[len(x_off)-1], BVA_accuracy[len(BVA_accuracy)-1]),
        xytext=(x_off[len(x_off)-1]+.025, BVA_accuracy[len(BVA_accuracy)-1]+.017), color = 'black', fontsize=textsize,
        arrowprops=dict(arrowstyle="->"))
        ax2.set_ylabel('Recovery rate', fontsize = textsize)
        ax2.tick_params(labelsize=textsize)
        ax2.legend(loc = 'upper center', bbox_to_anchor = (0.5,0.9), fontsize = textsize)
        ...
        ax2 = ax.twinx()
        ax2.plot(x_off, BVA_accuracy, 'lightgreen', marker = '^', markersize=10, markeredgecolor = 'g', markeredgewidth=0.8, label = 'BVA-Rer')
        ax2.scatter(x_off[len(x_off)-1]+wid, Decoding_accuracy, c = 'r', marker = 'x', s=70, label = 'Decoding-Rer')
        ax2.annotate('($\#$W/2, {:.2f})'.format(BVA_accuracy[0]),
        xy=(x_off[0], BVA_accuracy[0]),
        xytext=(x_off[0]-0.01, BVA_accuracy[0]-0.04), color = 'black', fontsize=textsize,
        arrowprops=dict(arrowstyle="->"))
        ax2.annotate('(offset, {:.2f})'.format(BVA_accuracy[len(BVA_accuracy)-1]),
        xy=(x_off[len(x_off)-1], BVA_accuracy[len(BVA_accuracy)-1]),
        xytext=(x_off[len(x_off)-1]+0.025, BVA_accuracy[len(BVA_accuracy)-1]+.015), color = 'black', fontsize=textsize,
        arrowprops=dict(arrowstyle="->"))
        ax2.set_ylabel('Recovery rate', fontsize = textsize)
        ax2.legend(loc = 'upper center', bbox_to_anchor = (0.5,0.9), fontsize = textsize)
    else:
        ...
        ax2.annotate('(\\#W/2, {:.2f})'.format(BVA_accuracy[0]),
        xy=(x_off[0], BVA_accuracy[0]),
        xytext=(x_off[0]-0.001, BVA_accuracy[0]-0.04), color = 'black', fontsize=textsize,
        arrowprops=dict(arrowstyle="->"))
        ax2.annotate('(offset, {:.2f})'.format(BVA_accuracy[len(BVA_accuracy)-1]),
        xy=(x_off[len(x_off)-1], BVA_accuracy[len(BVA_accuracy)-1]),
        xytext=(x_off[len(x_off)-1]+0.0025, BVA_accuracy[len(BVA_accuracy)-1]+0.015), color = 'black', fontsize=textsize,
        arrowprops=dict(arrowstyle="->"))
        ax2.set_ylabel('Recovery rate', fontsize = textsize)
        ax2.legend(loc = 'upper center', bbox_to_anchor = (0.5,0.9), fontsize = textsize)
    plt.tight_layout()
