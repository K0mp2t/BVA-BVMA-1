...
class Attack: 
    def __init__(self, chosen_kws, target_queries, observed_queries, kws_leak_percent, trend_matrix_norm, real_size, real_length, offset_of_Decoding):
        self.real_tag = {}
        self.recover_tag = {}
    ...
    def BVA_main(self, gamma):
        ...
        """
        observed_size_in_baseline = self.observed_size.copy()
        self.time = self.BVA_recover(observed_size_in_baseline, real_size_after_injection)
    ...
    def BVMA_main(self, query_type):
        ...
        self.BVMA_inject(real_size_after_injection, real_length_after_injection)
        e = time.time()
        self.inject_time = e - s
        """
        recovery
        ...
        self.time = self.BVMA_recover(query_type, baseline_trend_matrix, observed_size_in_baseline, observed_length_in_baseline, real_size_after_injection, real_length_after_injection)
        
        kws_each_doc = math.ceil(len(self.chosen_kws)/2)
        self.total_inject_length = math.ceil(np.log2(kws_each_doc + kws_each_doc))
        self.accuracy = self.recover_queries_num/self.total_queries_num
    def BVMA_recover(self, query_type, baseline_trend_matrix, observed_size_in_baseline, observed_length_in_baseline, real_size_after_injection, real_length_after_injection):
        self.real_tag = {}
        self.recover_tag = {}
        """
        ...
        for i_week in self.target_queries:
            for query in i_week:
                ...
                if self.recover_tag[query] == self.real_tag[query]:
                    self.recover_queries_num += 1
                self.total_queries_num += 1
    def BVMA_inject(self, real_size_after_injection, real_length_after_injection):       
        kws_each_doc = math.ceil(((int) (len(self.chosen_kws)*self.kws_leak_percent))/2)
        if kws_each_doc==0:
            num_injection_doc=0
        else:
    ...
    def SR_main(self, m):
        ...
        """
        observed_length_in_baseline = self.observed_length.copy()
        self.time = self.SR_recover(observed_length_in_baseline, real_length_after_injection)

        self.accuracy = self.recover_queries_num/self.total_queries_num
    def SR_recover(self, observed_length_in_baseline, real_length_after_injection):
        self.real_tag = {}
        self.recover_tag = {}
        for i_week in self.target_queries:
            for query in i_week:
    ...
    def SR_inject(self, real_length_after_injection):
        ...
        self.total_inject_length = self.m*len(self.chosen_kws)*self.kws_leak_percent
        self.total_inject_size = sum(sum(a) for a in size_each_doc)
        for kws_ind in range((int) (len(self.chosen_kws)*self.kws_leak_percent)):
